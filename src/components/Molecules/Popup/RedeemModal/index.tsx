...
type RedeemTier = {
  ...
  reward?:
    | { unit: "COIN"; amount: number }
    | { unit: "TEXT"; label: string };
};
...
export default function RedeemModal({
  ...
  const displayTiers = useMemo(
    () =>
      ...
      ]).map((t) => ({ tier: t, display: normalizeDisplay(t) })),
    [tiers]
  );
                ...
                <div className="w-full flex flex-col items-stretch gap-2 sm:gap-3 lg:gap-4 mt-2 sm:mt-4 lg:mt-6 px-2 sm:px-4 lg:px-6 max-w-2xl lg:max-w-3xl overflow-y-auto custom-scrollbar flex-1">
                  {displayTiers.map(({ tier, display }) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => tier.canRedeem && onRedeem(tier.id)}
                        disabled={!tier.canRedeem}
                      ...
                      >
                        <Image
                          src={tier.canRedeem ? "/Property 1=Đủ để đổi.svg" : "/Property 1=Chưa đủ để đổi.svg"}
