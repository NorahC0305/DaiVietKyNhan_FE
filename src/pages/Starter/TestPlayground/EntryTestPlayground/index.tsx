      ...
      <div className="flex items-center justify-center">
        ...
        >
          <Image
            ...
            className="absolute"
          />
          <p className="relative z-10 text-[#835D26] font-bold">Bắt đầu</p>
        </button>
      </div>
    </div>
  )
...
export default function EntryTestPlaygroundPage({
  testHome,
}: {
  testHome: ITestHome[];
}) {
  ...
  const handleNextQuestion = useCallback(
    (answerId: number) => {
      ...
      // Gọi API để save answer giống test chính, nhưng không cần xử lý lỗi nghiêm trọng
      ...
      if (currentQuestion && apiAnswer) {
        (async () => {
          try {
            ...
            // Không hiển thị error cho test playground, chỉ log
            console.log(
              "Playground test - answer not saved to backend:",
              error
            );
          }
        })();
